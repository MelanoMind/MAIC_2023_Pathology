...
def train_epoch(model, classifier,train_loader, optimizer, criterion, device):
    ...
    for i in range(1):
        print(f"Prob: {(y_prob[i].item()):.4f}, Label: {label[i].item()}, Loss: {(loss.item()):.2f}",end=' ')
    print(f"total Loss: {(total_loss/len(train_loader)):.2f} len: {len(train_loader)}",end=' ')
...
if __name__ == '__main__':
    ...
    train_list_0 = list(df_train['Slide_name'])[:100] # [:49]
    train_list_1 = list(df_train['Slide_name'])[-100:] # [-50:-1]
    
    valid_list = list(df_valid['Slide_name'])[:12]+list(df_valid['Slide_name'])[-12:] # list(df_valid['Slide_name'])[:10]+list(df_valid['Slide_name'])[-10:]
    ...
    model = timm.create_model('efficientnet_b3', pretrained=True, num_classes=512, drop_rate=DROP_RATE)
    print('model dim: ',model.classifier.in_features)
    ...
    for epoch in range(EPOCH):
        ...
        for number,(pkl,pkl2) in enumerate(zip(train_list_0,train_list_1)): # 두개의 길이가 같게 해야함..
            ...
            if number%3==0: # number%2==0:
                ...
                PATH = folder + '/' + pkl +'_patch.pkl'
                PATH2 =folder + '/' + pkl2+'_patch.pkl'
                train_dataset_4 = load_dataset_from_pickle(PATH)
                train_dataset_5 = load_dataset_from_pickle(PATH2)
                train_data = torch.utils.data.ConcatDataset([train_dataset_0, train_dataset_1,train_dataset_2,train_dataset_3,train_dataset_4,train_dataset_5])
        ...
        torch.save(model,f'./ENCODER_ifb3_ep{epoch}_v2.pt')
        torch.save(classifier,f'./CLS_ifb3_ep{epoch}_v2.pt')
