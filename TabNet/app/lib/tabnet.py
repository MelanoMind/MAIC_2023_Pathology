        ...
        if method == "means":
            ...
            for col in self.category_columns:
                ...
                if self.dataFrame[col].dtype == "object":
                    le = LabelEncoder()
                    self.dataFrame[col] = le.fit_transform(
            ...
            for col, le in label_encoders.items():
                self.dataFrame[col] = le.inverse_transform(
                    self.dataFrame[col].astype(int)
                )
            ...
            columns_to_reorder = [
                ...
                if col not in ["Patient_ID", "Recurrence"]
            ]

            self.dataFrame = self.dataFrame[
                ["Patient_ID"] + columns_to_reorder + ["Recurrence"]
            ]
            return self.dataFrame
        elif method == "None":
            return self.dataFrame
...
class TabularDataSet:
    def __init__(
        ...
        self.numerical_columns = numerical_columns
        self.dataFrame = FillMissingColumns(
            df, category_columns, numerical_columns
        ).fill_numerical_columns(method=fill_numerical_method)
        self.category_columns = category_columns
        self.category_dims = {}
        self.label_encoders = {}
...
class GetIndicesForCategoricalColumn:
    ...
    def get_indices(self):
        indices = [
            self.dataFrame.columns.get_loc(col)
...
class TransformDataToTabNet:
    def __init__(self, dataFrame, category_columns, numerical_columns, method):
        ...
        """
        df = TabularDataSet(
            dataFrame, category_columns, numerical_columns, method
        ).convertEncodedDataFrame(category_columns)
        ...
        self.categorical_indices = GetIndicesForCategoricalColumn(
