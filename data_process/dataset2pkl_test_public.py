...
# os.environ['OPENCV_IO_MAX_IMAGE_P?IXELS'] = str(2**64)
...
class MILDataset(Dataset): # 참고 https://github.com/MSKCC-Computational-Pathology/MIL-nature-medicine-2019/blob/master/MIL_train.py
    def __init__(self, slide_list, augmentation=False, preprocessing=False):
        self.slide_list = slide_list # 받는 슬라이드 이미지 이름. ex) test_001.png
        ...
        mask_path = f'../pickle_test_public_mask/dataset_{self.slide_list[0]}_mask.pkl'
    ...
    def __getitem__(self, idx):
        ...
        # print('##### len(self.slide_list), self.slide_list, img.shape : ', len(self.slide_list), self.slide_list, img.shape)
        return img
...
if __name__ == "__main__":
    ...
    test_list = df['Slide_name'].tolist()

    # test_list = test_list[:3]
    print(f'[test_list] {type(test_list)}, {len(test_list)}') # , {test_list}')
    
    # 전체 데이터 이미지 별로 pickle만들기
    for index, slide_name in enumerate(tqdm(test_list)):
        # print(index, slide_name)
        
        test_data = MILDataset([slide_name], augmentation=True, preprocessing=True)
        # print(f'    patchs : {type(test_data)}, {test_data}')
        print(f' ## [{index}] slide_name : {slide_name} | patchs : {len(test_data)}') 
        # for patch in test_data:
        #     print('  #', len(patch)) # 3 (torch.Size([3, 512, 512]))
        #     print(type(patch[0]), patch[0].shape) # <class 'torch.Tensor'> torch.Size([512, 512])
        #     print(type(patch[1]), patch[1].shape)
        #     print(type(patch[2]), patch[2].shape)
        # for i in range(len(test_data)):
        #     print('  #', len(test_data))
        #     print(type(test_data[i]), test_data[i].shape) # class 'torch.Tensor'> torch.Size([3, 512, 512])
        #     break
            
        patch_path = f'../pickle_test_public_patch/dataset_{slide_name}_patch.pkl'
        save_dataset_as_pickle(test_data, patch_path)
        
        print(f"  # File Size: (org img) {convert_size(os.path.getsize(f'../dataset/test_public/{slide_name}.png')), 'bytes'} | (mask) {convert_size(os.path.getsize(f'../pickle_test_public_mask/dataset_{slide_name}_mask.pkl')), 'bytes'} | (patch) {convert_size(os.path.getsize(patch_path)), 'bytes'}")
