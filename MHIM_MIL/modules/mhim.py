...
class MHIM(nn.Module):
    def __init__(self, mlp_dim=512, mask_ratio=0, n_classes=2, temp_t=1., temp_s=1., dropout=0.25, act='relu', select_mask=True, select_inv=False, msa_fusion='vote', mask_ratio_h=0., mrh_sche=None, mask_ratio_hr=0., mask_ratio_l=0., da_act='gelu', baseline='selfattn', head=8, attn_layer=0):
        ...
        # self.patch_to_emb = [nn.Linear(512, 512)] ## added
    ...
    def forward_teacher(self,x,return_attn=False):
        # print(' **[mhim]forward_teacher(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = torch.reshape(x, (x.shape[0], -1, 1024)) # added # x = torch.reshape(x, (x.shape[0], -1, x.shape[3]))
        # print(' **[mhim]forward_teacher(torch.reshape)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = self.patch_to_emb(x)
        # print(' **[mhim]forward_teacher(self.patch_to_emb(x))** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = self.dp(x)
        # print(' **[mhim]forward_teacher(self.dp(x))** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        # x = torch.reshape(x, (x.shape[0], -1, x.shape[3])) # added
        # print(' **[mhim]forward_teacher(torch.reshape)** ', type(x), x.shape) # added # <class 'torch.Tensor'> torch.Size([1, 1536, 512])
    ...
    def forward_test(self,x,return_attn=False,no_norm=False):
        # print(' **[mhim]forward_test(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = torch.reshape(x, (x.shape[0], -1, 1024)) # added # x = torch.reshape(x, (x.shape[0], -1, x.shape[3]))
        # print(' **[mhim]forward_test(torch.reshape)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = self.patch_to_emb(x)
        # print(' **[mhim]forward_test(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = self.dp(x)
        # print(' **[mhim]forward_test(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        # x = torch.reshape(x, (x.shape[0], -1, x.shape[3])) # added
        # print(' **[mhim]forward_test(x)** ', type(x), x.shape) # added # <class 'torch.Tensor'> torch.Size([1, 1536, 512])
    ...
    def pure(self,x,return_attn=False):
        x = self.patch_to_emb(x)
        # print(' **[mhim]pure(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = self.dp(x)
        # print(' **[mhim]pure(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        # x = torch.reshape(x, (x.shape[0], -1, x.shape[3])) # added
        # # print(' **[mhim]pure(x)** ', type(x), x.shape) # added # <class 'torch.Tensor'> torch.Size([1, 1536, 512)
        ps = x.size(1)
    ...
    def forward_loss(self, student_cls_feat, teacher_cls_feat):
        # print(" **[forward_loss]** ", student_cls_feat, teacher_cls_feat) # tensor([[-1.2710,  0.5287, -1.2727,  ..., -0.1164, -1.3161, -1.0523], ...], device='cuda:0', grad_fn=<SliceBackward0>) None
    ...
    def forward(self, x,attn=None,teacher_cls_feat=None,i=None):
        # print(' **[mhim]forward(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = torch.reshape(x, (x.shape[0], -1, 1024)) # added # x = torch.reshape(x, (x.shape[0], -1, x.shape[3]))
        # print(' **[mhim]forward(torch.reshape)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = self.patch_to_emb(x)
        # print(' **[mhim]forward(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        x = self.dp(x)
        # print(' **[mhim]forward(x)** ', type(x), x.shape) # <class 'torch.Tensor'> torch.Size([1, 3, 512, 512])
        # x = torch.reshape(x, (x.shape[0], -1, x.shape[3])) # added
        # print(' **[mhim]forward(x)** ', type(x), x.shape) # added # <class 'torch.Tensor'> torch.Size([1, 1536, 512)
        ps = x.size(1)
        # print(' **[mhim]forward(ps)** ', type(ps), ps)
